...
If you're connecting from Python, the community package [`df-s3-filen-wrapper`](https://pypi.org/project/df-s3-filen-wrapper/) gives you a boto3 client with these settings already applied. It also works around a few gateway differences, for example multipart uploads not being supported. To run the S3 server on Kubernetes, see [Using Kubernetes](/docs/cli#using-kubernetes).
...
-   `--s3-threads`: enables clustering, number of threads to use for the server (default is no clustering; explicitly set to 0 to set by CPU core count).
    If you experience rate-limiting using this, using an [auth config](/docs/cli/authenticating#auth-config) might help.
